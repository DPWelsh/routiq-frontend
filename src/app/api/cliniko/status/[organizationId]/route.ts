/**
 * Cliniko Status Proxy - PRODUCTION READY
...
export const GET = withClerkAuth(async (userId, request: NextRequest) => {
  try {
    ...
    const backendUrl = `${BACKEND_API_URL}/api/v1/cliniko/status/${organizationId}`

    console.log(`[DEBUG] Proxying cliniko status to: ${backendUrl}`)
    ...
    console.log(`[DEBUG] Cliniko status proxy successful: ${backendResponse.status}`)
    ...
    console.error(`[ERROR] Cliniko status proxy failed:`, error)

    return new Response(
      JSON.stringify({
        success: false,
        error: 'Failed to fetch cliniko status',
        code: 'CLINIKO_STATUS_PROXY_ERROR'
      }),
      {
        status: 500,
